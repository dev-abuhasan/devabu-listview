...
> extends ComponentCompat<P, S> {
	...
	constructor(props: P, context?: any) {
		...
		if (this.props.windowCorrectionConfig) {
			...
			this._windowCorrectionConfig = {
				applyToItemScroll:
					!!this.props.windowCorrectionConfig.applyToItemScroll,
				applyToInitialOffset:
					!!this.props.windowCorrectionConfig.applyToInitialOffset,
				value: windowCorrection,
			};
		} else {
			this._windowCorrectionConfig = {
			...
			if (uniqueKey) {
				...
				if (this.props.forceNonDeterministicRendering) {
					if (this._virtualRenderer) {
						const layoutManager =
							this._virtualRenderer.getLayoutManager();
						if (layoutManager) {
	...
	public scrollToIndex(index: number, animate?: boolean): void {
		...
		if (layoutManager) {
			...
			this.scrollToOffset(
				...
				offsets.y,
				animate,
				this._windowCorrectionConfig.applyToItemScroll
			);
		} else {
	...
	public bringToFocus(index: number, animate?: boolean): void {
		...
		if (itemLayout) {
			...
			const mainAxisListDimen = isHorizontal
				? listSize.width
				: listSize.height;
	...
	public scrollToOffset = (
		...
		useWindowCorrection: boolean = false
	): void => {
		if (this._scrollComponent) {
			if (this.props.isHorizontal) {
				y = 0;
				x = useWindowCorrection
					? x - this._windowCorrectionConfig.value.windowShift
					: x;
			} else {
				x = 0;
				y = useWindowCorrection
					? y - this._windowCorrectionConfig.value.windowShift
					: y;
			}
			...
			setTimeout(() => {
				if (this._pendingScrollToOffset) {
					...
					this.scrollToOffset(
						...
						offset.y,
						false,
						this._windowCorrectionConfig.applyToInitialOffset
					);
	...
	private _checkAndChangeLayouts(
		...
		if (
			...
			if (
				newProps.dataProvider.getSize() > this.props.dataProvider.getSize()
			) {
	...
	private _onSizeChanged = (layout: Dimension): void => {
		...
		this._layout.width = this.props.maxWidth
			? layout.width > this.props.maxWidth
				? this.props.maxWidth
				: layout.width
			: layout.width;
	...
	private _renderRowUsingMeta(itemMeta: RenderStackItem): JSX.Element | null {
		...
		if (!ObjectUtil.isNullOrUndefined(dataIndex) && dataIndex < dataSize) {
			...
			const type =
				this.props.layoutProvider.getLayoutTypeForIndex(dataIndex);
			...
			return (
				<ViewRenderer
					...
					baseViewStyle={this.props.baseViewStyle}
	...
	private _processOnEndReached(): void {
		if (this.props.onEndReached && this._virtualRenderer) {
			...
			const viewabilityTracker =
				this._virtualRenderer.getViewabilityTracker();
			if (viewabilityTracker) {
				...
				const listLength = this.props.isHorizontal
					? this._layout.width
					: this._layout.height;
				const triggerOnEndThresholdRelative =
					listLength *
					Default.value<number>(
						this.props.onEndReachedThresholdRelative,
						0
					);
				const triggerOnEndThreshold = Default.value<number>(
