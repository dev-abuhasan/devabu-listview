...
var RecyclerListView = /** @class */ (function (_super) {
    ...
    function RecyclerListView(props, context) {
        ...
        _this.scrollToOffset = function (x, y, animate, useWindowCorrection) {
            ...
            if (_this._scrollComponent) {
                if (_this.props.isHorizontal) {
                    y = 0;
                    x = useWindowCorrection
                        ? x - _this._windowCorrectionConfig.value.windowShift
                        : x;
                }
                else {
                    x = 0;
                    y = useWindowCorrection
                        ? y - _this._windowCorrectionConfig.value.windowShift
                        : y;
                }
        ...
        _this._onSizeChanged = function (layout) {
            ...
            _this._layout.width = _this.props.maxWidth
                ? layout.width > _this.props.maxWidth
    ...
    RecyclerListView.prototype.bringToFocus = function (index, animate) {
        ...
        var currentScrollOffset = this.getCurrentScrollOffset() +
            this._windowCorrectionConfig.value.windowShift;
        var isHorizontal = this.props.isHorizontal;
        if (itemLayout) {
            var mainAxisLayoutDimen = isHorizontal
                ? itemLayout.width
                : itemLayout.height;
            var mainAxisLayoutPos = isHorizontal ? itemLayout.x : itemLayout.y;
            var mainAxisListDimen = isHorizontal
                ? listSize.width
                : listSize.height;
            var screenEndPos = mainAxisListDimen + currentScrollOffset;
            if (mainAxisLayoutDimen > mainAxisListDimen ||
    ...
    RecyclerListView.prototype._getWindowCorrection = function (offsetX, offsetY, props) {
        return ((props.applyWindowCorrection &&
            props.applyWindowCorrection(offsetX, offsetY, this._windowCorrectionConfig.value)) ||
    ...
    RecyclerListView.prototype._processOnEndReached = function () {
        if (this.props.onEndReached && this._virtualRenderer) {
            ...
            if (viewabilityTracker) {
                ...
                var listLength = this.props.isHorizontal
                    ? this._layout.width
                    : this._layout.height;
                var triggerOnEndThresholdRelative = listLength *
                    ts_object_utils_1.Default.value(this.props.onEndReachedThresholdRelative, 0);
                var triggerOnEndThreshold = ts_object_utils_1.Default.value(this.props.onEndReachedThreshold, 0);
                if (threshold <= triggerOnEndThresholdRelative ||
                    threshold <= triggerOnEndThreshold) {
